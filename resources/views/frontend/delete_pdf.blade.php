<x-app-layout>
    ...
    <div class="form-container">
        ...
        <form method="POST" action="{{ route('pdf.delete.process') }}" enctype="multipart/form-data" x-data="{ fileName: '' }">
            @csrf

            {{-- File upload --}}
            <div class="form-group">
                <label for="pdf" class="mb-2 block">{{ __('delete_pdf.select_pdf') }}</label>

                <div class="upload-row bg-gray-100 border border-gray-300 rounded px-3 py-2">
                    <button type="button"
                            @click="$refs.fileInput.click()"
                            class="custom-upload-btn">
                        {{ __('delete_pdf.choose_button') }}
                    </button>

                    <span x-text="fileName || '{{ __('delete_pdf.no_file') }}'" class="file-info truncate"></span>
                </div>
            ...
            <div class="form-group">
                ...
                <input type="text" id="pages" name="pages" placeholder="{{ __('delete_pdf.pages_placeholder') }}" required>
            </div>
