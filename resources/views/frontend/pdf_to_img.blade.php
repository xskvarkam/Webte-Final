<x-app-layout>
    ...
    <div class="form-container">
        ...
        <form method="POST" action="{{ route('pdf.to_img.process') }}" enctype="multipart/form-data" x-data="{ fileName: '' }">
            @csrf

            <div class="form-group">
                <label class="mb-2 block">{{ __('pdf_to_img.select_pdf') }}</label>

                <div class="upload-row bg-gray-100 border border-gray-300 rounded px-3 py-2">
                    <button type="button"
                            @click="$refs.fileInput.click()"
                            class="custom-upload-btn">
                        {{ __('pdf_to_img.choose_button') }}
                    </button>

                    <span x-text="fileName || '{{ __('pdf_to_img.no_file') }}'" class="file-info truncate"></span>
                </div>
